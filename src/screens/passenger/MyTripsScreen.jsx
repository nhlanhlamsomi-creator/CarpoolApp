...
const ChatIcon = () => (
  <Svg width="14" height="14" viewBox="0 0 24 24" fill="none">
    <Path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" stroke={colors.primary} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
  </Svg>
);

const XIcon = () => (
  <Svg width="14" height="14" viewBox="0 0 24 24" fill="none">
    <Path d="M18 6L6 18M6 6l12 12" stroke={colors.error} strokeWidth="2" strokeLinecap="round"/>
  </Svg>
);
...
export default function MyTripsScreen({ navigation }) {
  ...
  const handleCancel = (id) => {
    Alert.alert(
      'Cancel Booking?',
      'A R10 fee applies if cancelled more than 5 minutes after booking.',
      [
      ...
      <View style={styles.cardBottom}>
        <View style={[
          ...
          <View style={styles.badgeInner}>
            {item.status === 'confirmed' && <ClockIcon />}
            {item.status === 'completed' && <CheckIcon />}
            {item.status === 'cancelled' && <XIcon />}
            <Text style={[
              styles.badgeText,
...
const styles = StyleSheet.create({
  ...
  metaText:       { fontSize: typography.fontSize.sm, color: colors.textMuted },
  cardBottom:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  badge:          { borderRadius: radius.full, paddingHorizontal: spacing.md, paddingVertical: 5 },
  ...
  chatBtn:        { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: colors.primaryLight, borderRadius: radius.full, paddingHorizontal: spacing.md, paddingVertical: 8 },
  chatBtnText:    { color: colors.primary, fontWeight: '700', fontSize: typography.fontSize.sm },
  cancelBtn:      { flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1.5, borderColor: colors.error, borderRadius: radius.full, paddingHorizontal: spacing.md, paddingVertical: 8 },
