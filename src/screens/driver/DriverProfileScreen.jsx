...
export default function DriverProfileScreen({ navigation }) {
  ...
  const handleLogout = () => {
    Alert.alert('Log out?', 'You will need to log in again to access your account.', [
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.sub}>Manage your driver account</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.avatarSection}>
          ...
          <View style={[styles.badge, user?.verified ? styles.badgeVerified : styles.badgePending]}>
            <Text style={styles.badgeText}>{user?.verified ? '✓ Verified Driver' : '⏳ Verification Pending'}</Text>
          </View>
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          {[
            { l: 'Make & Model', v: `${driverDetails.vehicleMake || '-'} ${driverDetails.vehicleModel || ''}` },
            { l: 'Colour',       v: driverDetails.vehicleColor   || '-' },
            { l: 'Plate',        v: driverDetails.licensePlate   || '-' },
            { l: 'Licence No.',  v: driverDetails.licenceNumber  || '-' },
          ].map(r => (
            <View key={r.l} style={styles.infoRow}>
              <Text style={styles.infoLabel}>{r.l}</Text>
              <Text style={styles.infoValue}>{r.v}</Text>
        ...
        <View style={styles.infoCard}>
          ...
          {[
            { l: 'Total Trips Offered', v: '0' },
            { l: 'Active Passengers',   v: '0' },
            { l: 'Shift hours today',   v: '0h' },
          ].map(r => (
            <View key={r.l} style={styles.infoRow}>
              <Text style={styles.infoLabel}>{r.l}</Text>
              <Text style={styles.infoValue}>{r.v}</Text>
...
const styles = StyleSheet.create({
  ...
  badge:        { borderRadius: radius.full, paddingHorizontal: spacing.md, paddingVertical: 4, marginTop: spacing.sm },
  badgeVerified:{ backgroundColor: colors.primaryLight },
  badgePending: { backgroundColor: '#FFF8E1' },
  badgeText:    { fontSize: typography.fontSize.xs, fontWeight: '700', color: colors.primaryDark },
  ...
  infoValue:    { color: colors.textPrimary, fontWeight: '500', fontSize: typography.fontSize.sm, maxWidth: '55%', textAlign: 'right' },
